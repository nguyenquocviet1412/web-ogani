...
<div class="container">
    <h2>Sửa thông tin nhân viên bị cấm</h2>
    <form action="{{ route('userbans.update', $userban->id) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="name">Họ và Tên:</label>
            <input type="text" name="name" class="form-control" value="{{ $userban->name }}" required>
        </div>

        <div class="form-group">
            <label for="Role_id">Chức vụ:</label>
            <select name="Role_id" class="form-control">
                <option value="1" {{ $userban->Role_id == 1 ? 'selected' : '' }}>Quản trị</option>
                <option value="3" {{ $userban->Role_id == 3 ? 'selected' : '' }}>Nhân viên</option>
            </select>
        </div>

        <div class="form-group">
            <label for="note">Lý do cấm:</label>
            <textarea name="note" class="form-control" rows="3">{{ $userban->note }}</textarea>
        </div>

        <div class="form-group">
            <label for="active">Tình trạng:</label>
            <select name="active" class="form-control">
                <option value="0" {{ $userban->active == 0 ? 'selected' : '' }}>Khóa tài khoản</option>
                <option value="1" {{ $userban->active == 1 ? 'selected' : '' }}>Sa thải</option>
            </select>
        </div>
